...
new_production_table <- new_production_table[-c(68, 69, 70, 71, 93, 189, 210, 211, 224),]
...
new_consumption_table <- new_consumption_table[-c(68, 69, 70, 71, 93, 189, 210, 211, 224),]
...
for(i in 2:ncol(production)) {
  for(k in 1:228) {
    if(as.numeric(production[i,k]) > mx) {
      mx = as.numeric(production[i,k])
    }
  }
}
...
##### Adding per-capita functionality

# Import population table
pop_table <- read.csv("Documents/Data Science Class/Project #3/worldPopulation.csv", stringsAsFactors = FALSE)

# Remove extra columns at the beginning
pop_table$Series.Name <- NULL
...
#Create the circle colors for the per-capita production and consumption
  ...
  prod_2016 <- filter(capita_production_table, capita_production_table$Year == "2016")
  
  cons_2016 <- filter(capita_consumption_table, capita_consumption_table$Year == "2016")
